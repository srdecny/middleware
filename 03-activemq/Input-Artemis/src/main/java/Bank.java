...
public class Bank implements MessageListener {
	...
	// text message command return balance of the account
	public static final String BALANCE_CHECK_MSG = "BALANCE_CHECK";
	...
	// report type "not enough money to do the transfer"
	public static final int REPORT_TYPE_SMALL_BALANCE = 2;
	...
	// map account numbers to balances
	// starting with 5000 balance
	...
	private void processTextMessage(TextMessage txtMsg) throws JMSException {
		...
		if (NEW_ACCOUNT_MSG.equals(txtMsg.getText())) {
			...
			if (clientAccounts.get(clientName) != null) {
				...
				balances.put(accountNumber, 5000);
			}
		...
		} else if (BALANCE_CHECK_MSG.equals(txtMsg.getText())) {
			Number accountNumber = txtMsg.getIntProperty(Client.ACCOUNT_NUMBER_PROPERTY);
			TextMessage replyMessage = bankSession.createTextMessage();
			// Assuming the account number exists
			replyMessage.setIntProperty(Client.BALANCE_PROPERTY, balances.get(accountNumber));
	...
	private void processMapMessage(MapMessage mapMsg) throws JMSException {
		...
		if (order == ORDER_TYPE_SEND) {
			...
			synchronized(this) {
				...
				// Does the sender have enough cash money?
				if (balances.get(clientAccount) >= amount) {
					...
					// Update the balances
					balances.put(clientAccount, balances.get(clientAccount) - amount);
					balances.put(destAccount, balances.get(destAccount) + amount);	
					// send report to receiver client's destination
				} else {
					// Not enough balance, notify the receiver
					// The sender could be notified instead; but sending the error to the receiver will keep things simple
					System.out.println("Account " + clientAccount + " attempted to transfer $" + amount + " but had only $" + balances.get(clientAccount));
					reportMsg.setInt(REPORT_TYPE_KEY, REPORT_TYPE_SMALL_BALANCE);
				}
